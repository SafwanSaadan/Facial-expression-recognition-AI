...
class GUI:
    def __init__(self, root):
        ...
        # إضافة مثيل لـ FaceMeshDetector
        self.face_mesh_detector = fs.FaceMeshDetector()
    ...
    def update_camera(self):
        while True:
            if self.camera_running:
                ...
                # تحليل الوجوه باستخدام FaceMeshDetector
                if self.show_webcam:
                    image, faces = self.face_mesh_detector.findFaceMesh(image)
                    # رسم نقاط الوجه
                    for face in faces:
                        for x, y in face:
